...
- [ ] `org.jetbrains.kotlin.android` removed from all build files and version catalog (built-in
  Kotlin replaces it)
...
- [ ] No `android.builtInKotlin=false` unless required by incompatible plugin (documented as
  temporary)
...
- [ ] `androidResources { enable = true }` present if module uses Android or Compose Multiplatform
  resources
...
- [ ] Unique `namespace` for each library module (different from app module;
  `android.uniquePackageNames=true` is default in AGP 9.0)
...
- [ ] Any opt-out flags (`android.newDsl=false`, `android.builtInKotlin=false`) documented with
  reason
...
- [ ] `xcodebuild -project iosApp/*.xcodeproj -scheme <scheme> -sdk iphonesimulator build`
  succeeds (if iOS app exists)
- [ ] Desktop app compiles: `./gradlew :desktopApp:run` or equivalent (if desktop target exists)
- [ ] Web/Wasm target compiles: `./gradlew :wasmJsApp:wasmJsBrowserDistribution` or equivalent (if
  web target exists)

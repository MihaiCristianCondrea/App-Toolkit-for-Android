...
// Each device animation is bundled in one direction only. The reverse is the same animation played
// backwards, which the replay-mode control below reaches through ToolkitIconReplayMode.Reverse, so a
// second drawable per device would ship the same motion twice. anim_media_play covers pause the same
// way.
private val animationSamples = listOf(
    ...
    "anim_volume_expand" to DesignSystemR.drawable.anim_volume_expand,
)

...
| **Integration** | `:sample:integration:*` | External SDK/service bridge (e.g., Ads). |
...
Integration modules act as a bridge between the application and external SDKs or services.
- `:sample:integration:ads`: Connects the sample app's ad unit IDs to the library's ad infrastructure.

## Dependency Rules

To maintain a healthy architecture, we enforce the following rules via the `ModuleBoundariesPlugin`:

1.  **Core/Integration cannot depend on Feature**: Infrastructure and integrations should not know about high-level features.
...
3.  **Core/Feature/Integration cannot depend on App**: The app module is the composition root and should be at the top of the dependency graph.
